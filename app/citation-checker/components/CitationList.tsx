...
import { getCitationTypeColor, badgeStyles, cardStyles, cn } from "@/lib/styles"
...
export function CitationList({ 
  ...
  const itemClass = cn(
    'flex gap-2 bg-white rounded border border-gray-200',
    compact 
      ? 'items-center p-1.5'
      : 'items-start p-2 hover:border-gray-300 transition-colors'
  )
  const badgeClass = compact ? badgeStyles.compact : badgeStyles.base
  const textClass = compact 
    ? 'text-xs text-gray-600 flex-1 truncate'
    : 'text-sm text-gray-700 flex-1 break-words'
  const moreClass = compact 
    ? 'text-xs text-gray-500 text-center pt-1' 
    : 'text-sm text-gray-500 text-center pt-1'
    ...
    <div className={containerClass}>
      ...
      <div className={listClass}>
        {displayCitations.map((citation: any, index: number) => {
          ...
          const colorClass = getCitationTypeColor(type)
          ...
          return (
            <div 
              ...
              className={itemClass}
            >
              <span className={cn(badgeClass, colorClass)}>
